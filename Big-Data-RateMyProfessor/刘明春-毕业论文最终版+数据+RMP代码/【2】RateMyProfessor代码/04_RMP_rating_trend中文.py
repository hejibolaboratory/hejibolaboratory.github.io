...
def professor_rating_year_trend(file):
    ...
    ax1.set_xlim([0, 19])
    ax1.set_ylabel("教授综合得分平均分", fontsize=14)
    ax1.plot(list(x), list(df_mean), color="black", lw=1)
    ax1.fill_between(x, df_mean - df_se, df_mean + df_se, color="grey",lw=5) #显示标准区间
    plt.xlabel("教授教龄", fontsize=14)
    plt.yticks(size=14)
    plt.xticks( size=14)
    ...
    ax1.text(1.5, 1.5, r"助理教授", fontsize=14)
    ax1.text(8, 1.5, r'副教授', fontsize=14)
    ax1.text(15,1.5, r'教授', fontsize=14)
    ax1.set_ylim([1.0, 5.0])

    # 显示教授人数
    ax2 = ax1.twinx()
    plt.ylabel('教授数量', fontsize=14)
    ...
    ax2.plot(list(x), list(sum), color="black") # 教授人数的变化趋势，代替柱状图

    fig.tight_layout() #自动调整子图参数，使之填充整个图像区域 https://www.jianshu.com/p/91eb0d616adb
    plt.yticks(size=14)
    plt.show()
    # plt.draw()
    # plt.savefig('教授教龄变化.png', dpi=300)
